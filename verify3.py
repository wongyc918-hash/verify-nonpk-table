...
INPUT_EXCEL    = "tables_to_verify1.xlsx"
OUTPUT_EXCEL   = "verification_results.xlsx"
MAX_WORKERS    = 4

CHUNK_SIZE      = 100_000   # rows fetched per chunk — controls memory usage
...
def compute_hash_counter(engine, table_name, columns, total_rows=None):
    """
    Stream the full table in chunks of CHUNK_SIZE rows.
    Normalize each row to strings and return Counter(row_hash -> occurrence_count).
...
def fetch_rows_matching_hashes(engine, table_name, columns, target_hashes, limit, total_rows=None):
    """
    Re-scan the table in chunks and collect rows whose normalized hash is in
    target_hashes.  Stops early once `limit` rows are collected.
    """
    query        = f"SELECT {', '.join(columns)} FROM {table_name}"
    collected    = []
    matched_total = 0
